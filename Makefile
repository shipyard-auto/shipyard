...
LDFLAGS := \
  -X github.com/shipyard-auto/shipyard/internal/app.Version=$(VERSION) \
...
FAIRWAY_LDFLAGS := \
  -X github.com/shipyard-auto/shipyard/addons/fairway/internal/app.Version=$(VERSION) \
...
package: dist
	tar -C $(DIST_DIR)/linux-amd64  -czf $(DIST_DIR)/$(APP_NAME)_$(VERSION)_linux_amd64.tar.gz  $(APP_NAME)
	tar -C $(DIST_DIR)/linux-arm64  -czf $(DIST_DIR)/$(APP_NAME)_$(VERSION)_linux_arm64.tar.gz  $(APP_NAME)
	tar -C $(DIST_DIR)/darwin-amd64 -czf $(DIST_DIR)/$(APP_NAME)_$(VERSION)_darwin_amd64.tar.gz $(APP_NAME)
	tar -C $(DIST_DIR)/darwin-arm64 -czf $(DIST_DIR)/$(APP_NAME)_$(VERSION)_darwin_arm64.tar.gz $(APP_NAME)
...
package-fairway: dist-fairway
	tar -C $(DIST_DIR)/fairway-linux-amd64  -czf $(DIST_DIR)/$(FAIRWAY_APP_NAME)_$(VERSION)_linux_amd64.tar.gz  $(FAIRWAY_APP_NAME)
	tar -C $(DIST_DIR)/fairway-linux-arm64  -czf $(DIST_DIR)/$(FAIRWAY_APP_NAME)_$(VERSION)_linux_arm64.tar.gz  $(FAIRWAY_APP_NAME)
	tar -C $(DIST_DIR)/fairway-darwin-amd64 -czf $(DIST_DIR)/$(FAIRWAY_APP_NAME)_$(VERSION)_darwin_amd64.tar.gz $(FAIRWAY_APP_NAME)
	tar -C $(DIST_DIR)/fairway-darwin-arm64 -czf $(DIST_DIR)/$(FAIRWAY_APP_NAME)_$(VERSION)_darwin_arm64.tar.gz $(FAIRWAY_APP_NAME)

checksums-fairway: package-fairway
	cd $(DIST_DIR) && shasum -a 256 \
		$(FAIRWAY_APP_NAME)_$(VERSION)_linux_amd64.tar.gz \
		$(FAIRWAY_APP_NAME)_$(VERSION)_linux_arm64.tar.gz \
		$(FAIRWAY_APP_NAME)_$(VERSION)_darwin_amd64.tar.gz \
		$(FAIRWAY_APP_NAME)_$(VERSION)_darwin_arm64.tar.gz \
		> $(FAIRWAY_APP_NAME)_$(VERSION)_checksums.txt
